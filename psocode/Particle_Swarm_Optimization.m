...
%}
format long;
bird=reshape(1:(max_iteration+1)*Bird_in_swarm*Number_of_quality_in_Bird,Bird_in_swarm, Number_of_quality_in_Bird, max_iteration+1);
for i=1:Number_of_quality_in_Bird
    for j=1:max_iteration+1
        bird(:,i,j)=bird_min_range(i)+(bird_max_range(i)-bird_min_range(i))*rand(Bird_in_swarm,1);
    end
end
Vmax=bird_max_range*velocity_clamping_factor;
Vmin=-Vmax;

Velocity=reshape(1:(max_iteration+1)*Bird_in_swarm*Number_of_quality_in_Bird,Bird_in_swarm, Number_of_quality_in_Bird, max_iteration+1);
for i=1:Number_of_quality_in_Bird
    for j=1:max_iteration+1
        Velocity(:,i,j)=Vmin(i)+(Vmax(i)-Vmin(i))*rand(Bird_in_swarm,1);
    end
end
z=1;
figure();
    ...
    for p=1:Bird_in_swarm
        ...
        switch (availability_type)
            case 'min'
                ...
                if(availability(p, itr)==pBest_availability)
                    pBest=parameter;
                endif
        ...
        Velocity(p,:,(itr+1))=w(itr)*Velocity(p,:,itr) + social_constant*rand()*(gBest-parameter) + cognitive_constant*rand()*(pBest-parameter);
